...
TARGET_COMMANDS = ("validate-config", "generate-candidates", "train", "predict", "assign", "evaluate", "run")

def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(prog="mapel-linkage", description="Linkage Engine pre-alpha documentation scaffold.")
    parser.add_argument("--version", action="version", version=f"%(prog)s {__version__}")
    subparsers = parser.add_subparsers(dest="command")
    status = subparsers.add_parser("status", help="Show implementation status.")
    status.set_defaults(handler=_status)
    for command in TARGET_COMMANDS:
        subparser = subparsers.add_parser(command, help="Target interface; not implemented yet.")
...
def _not_implemented(namespace: argparse.Namespace) -> int:
    print(f"ERROR ML-PREALPHA-001: '{namespace.command}' is a target interface and is not implemented.")
    return 2
