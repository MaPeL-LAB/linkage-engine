...
| I1 — general orchestration | In progress | artifact stages, protected portfolio training, train/approve/infer separation, approved recipe inference, M3–M7 orchestration | every claimed workflow has a synthetic end-to-end artifact-chain test |
...
| B1 — synthetic benchmark library | Contracts implemented; corpus generation pending | experimental matrix, scenario families, aggregate registry, portfolio runs, failure retention, prospective family holdouts | registry coverage/readiness report and stable benchmark-critical schemas |
| I2B — nearest-family advisor | Deferred | similarity retrieval, coverage, OOD detection, uncertainty, abstention | held-out-family evaluation outperforms fixed-pipeline baselines |
| I2C — meta-learning ranker | Deferred | learned pipeline ranking on observable task features | improves on I2B under group-held-out validation and calibrated uncertainty |
| I2D — active benchmark planning | Deferred | prospective experiment selection for coverage gaps | demonstrates reduction in uncertainty or regret while preserving a fixed core benchmark |
