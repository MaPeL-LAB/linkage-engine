...
| M1 — safe configuration foundation | Merged | Pydantic schema, loader, compiler, registries, path policy, safe errors/logging, manifests, synthetic generator | malicious/invalid config tests, parser-complexity guards, privacy sentinels, Python 3.12 CI, and package-build checks pass |
| M2 — complete two-source synthetic slice | In progress through M2E | normalization, anchors, blocking, comparisons, Splink, XGBoost classifier/ranker, calibration, one-to-one no-match assignment, decisions, evaluation | deterministic synthetic `run` and all acceptance gates pass |
...
M1 through M2D are merged. M2E is a reviewable implementation candidate. Passing CI does not establish operational linkage validity.
